...
| §5.4 requirement | Bead / enforcement |
| --- | --- |
...
| Provider SDK/API-key imports are forbidden | hp-ara; `scripts/providerlint/check-provider-sdks.ts` |
...
## Daemon Release Verification

Bootstrap and upgrade flows must install daemon binaries only after the release
bundle passes the verifier in `apps/daemon/internal/release`.

The verifier requires:

- release manifest checksum matching the daemon binary SHA-256
- Ed25519 detached signature from a pinned release key and signing identity
- provenance attestation with matching source commit, builder identity,
  workflow path/ref, SLSA level 3 or higher, reproducible build marker, and
  subject digest
- SBOM listing dependencies plus the last-known-good CVE database generated by
  the release process

Checksum, signature, malformed SBOM, and provenance failures refuse install. An
insecure development override can bypass those failures only when actor, reason,
timestamp, and audit write all succeed. Diagnostics must render the persistent
banner:

> Daemon installed with provenance verification BYPASSED. Production use is unsupported.

High or critical SBOM findings do not silently pass. They require explicit user
acknowledgement, are audited, and are recorded in
`~/.hoopoe/daemon-inventory.json` with the finding list so Diagnostics can show
the exact reason the install is warning-marked.

Successful verification records the daemon version, checksum, SBOM digest,
attestation digest, source commit, builder identity, workflow path/ref, signing
identity, signing key id, compatibility floors, and verification status in the
daemon inventory. That inventory is the local Diagnostics source for "what is
running and where it came from"; it is not a replacement for the signed release
manifest.
