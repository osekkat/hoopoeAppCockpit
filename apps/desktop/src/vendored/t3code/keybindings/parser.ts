...
//
// Lifted from t3code `apps/server/src/keybindings.ts` lines 85–290:
// `parseKeybindingShortcut` (`cmd+shift+p` → KeybindingShortcut) and the
// recursive-descent `parseKeybindingWhenExpression` (`a && !(b || c)` →
// KeybindingWhenNode AST). The Effect-framework wrapping the original code
// uses for layering and dependency injection is intentionally NOT lifted
// here; the caller in `apps/desktop/src/main/keybindings/` provides any
// such plumbing.
