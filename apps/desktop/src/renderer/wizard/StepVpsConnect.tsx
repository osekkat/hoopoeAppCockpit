...
export interface StepVpsConnectProps {
  /** Default key fingerprint surfaced from the ssh_key step. The user
...
export function StepVpsConnect({
  ...
  function validate(input: FormState): { ok: true; profile: VpsConnectFormInput } | { ok: false; errors: Partial<Record<keyof FormState, string>> } {
    ...
    const portNum = Number.parseInt(input.port, 10);
