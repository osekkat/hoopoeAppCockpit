...
| Re-fetch desktop local clone from origin | Rehydrates the read-only mirror through the normal origin-sync path. Never runs `git reset`, `git clean`, checkout rewrites, or discard-style repair against the desktop clone; dirty mirrors are marked untrusted until they can be replaced from origin without overwriting local edits. |
...
- local clone cache can be re-fetched/rebuilt from origin without reset/clean repair against the desktop mirror;
...
**Vendoring layout.** Copied or adapted t3code implementation files land under `apps/desktop/src/vendored/t3code/` with the MIT notice preserved at the top of each copied/adapted file. A small set of Hoopoe-owned support files also lives there when it exists only to compile, export, document, or test the lifted helpers; those files are explicitly marked Hoopoe-owned and do not carry the upstream source notice. Adaptations (rebranding, rewiring) happen in our own files that import from `vendored/`. Do not edit copied/adapted upstream files in place except for mechanical mass renames — keep the diff against upstream small enough to re-merge later if needed.
...
| `apps/server/src/serverSettings.ts` (subset)                             | `apps/desktop/src/vendored/t3code/settings/stripDefaults.ts`                                                                                                             | Lifted as the deep-merge / stripDefaults helpers                                                                                                                                                                                                                                         | ✅      |
| —                                                                        | `apps/desktop/src/vendored/t3code/settings/index.ts`                                                                                                                     | Hoopoe-owned barrel re-export for the lifted settings helpers; no upstream MIT source header                                                                                                                                                                                              | ✅      |
...
| —                                                                        | `apps/desktop/src/vendored/t3code/_shims.ts`                                                                                                                             | Hoopoe-owned shim layer that lets the vendored modules compile under our plain-TS rules without pulling in `effect/`; no upstream MIT source header                                                                                                                                        | ✅      |
| —                                                                        | `apps/desktop/src/vendored/t3code/*.test.ts` + `apps/desktop/src/vendored/t3code/keybindings/*.test.ts`                                                                  | Hoopoe-owned regression tests for the lifted helpers; no upstream MIT source header                                                                                                                                                                                                       | ✅      |
...
| `scripts/release-smoke.ts`                                               | —                                                                                                                                                                        | Replaced by Hoopoe's Bun/Playwright smoke suites (`apps/desktop/tests/smoke/**`) and `scripts/e2e/run-e2e.ts`; no standalone `scripts/release-smoke.ts` is lifted on disk                                                                                                                   | ☑ — replaced, not lifted |
| `.github/workflows/release.yml`                                          | `.github/workflows/release.yml`                                                                                                                                          | Strip Linux + Windows matrix entries (keep mac arm64 + x64). Update secrets (CSC_LINK, APPLE_API_KEY, GH_TOKEN). Rename workflow to Hoopoe Release                                                                                                                                         | ✅      |
| `.github/workflows/*.yml` (typecheck, lint, test)                        | `.github/workflows/ci.yml` + `.github/workflows/schemas-codegen-drift.yml`                                                                                               | Adapt to Hoopoe command names; add PR/push lint/typecheck/test/e2e gate plus schema/preload codegen drift gate                                                                                                                                                                             | ✅      |
...
| `electron-builder` config (in `apps/desktop/package.json` / `build.yml`) | `scripts/build-desktop-artifact.ts` generated config + `apps/desktop/package.json`                                                                                       | Update `productName`, `appId`, signing/notarization settings; Hoopoe synthesizes the electron-builder config in the build artifact script instead of carrying a separate `build.yml`                                                                                                      | ✅      |
...
Add to top of every copied or adapted t3code implementation file:
...
Hoopoe-authored support files inside `apps/desktop/src/vendored/t3code/`
(`_shims.ts`, `settings/index.ts`, test files, and this vendored README) do
